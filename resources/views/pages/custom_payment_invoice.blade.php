                ...
                <div class=""
                    ...
                    <div>
                        ...
                        {{-- @if ($order->attachments)
                            <a href="{{ asset('public/vendor/orders/attachments/' . $order->attachments) }}"
                                class="anchor-deco" download="file"> <i class="fa fa-download"></i>
                                Download PDF</a>
                        @endif --}}
            ...
            <div class="col-md-9">
                ...
                <div class="card">
                    ...
                    <form id="order-place">
                            ...
                            <div class="row">
                                <div class="col-sm-6">
                                    <div class="form-group mb-0">
                                        <label for=""><Strong> Vendor : {{ $vendor->name }}</Strong></label>
                                        <br>
                                        <label for="">

                                            {{ $vendorAddress ? $vendorAddress->address_line1 : '' }} ,
                                            {{ $vendorAddress ? $vendorAddress->address_line2 : '' }}
                                        </label> <br>
                                        <label for="">{{ $vendorAddress ? $vendorAddress->city : '' }}</label>
                                        <br>
                                        <label
                                            for="">{{ $vendorAddress ? $vendorAddress->obj_state : '' }}</label>
                                        <br>
                                        <label
                                            for="">{{ $vendorAddress ? $vendorAddress->postal_code : '' }}</label>
                                        <br>
                                        <label
                                            for="">{{ $vendorAddress ? $vendorAddress->obj_country : '' }}</label>
                                        <br>
                                ...
                                <div class="col-sm-6 desktop-text-end">
                                    <div>
                                        <small>Invoice no {{ $order->invoice_number }}</small> <br>
                        ...
                        <div class="css-1gyuotj-grid_container mt-2"   style="display: flex; column-gap: 20px; flex-wrap: wrap; padding: 5px 10px;">
                            {{-- <span class=""><i class="fa fa-phone"></i> {{ $address->contact_no }}</span> --}}
            ...
            <div class="card">
                ...
                <div class="card-body card-padding  center">
                    ...
                    <div style="display: grid; gap: 10px">
                        <a href="{{ route('custom-invoice-payment.verify', ['pay' => $order->id]) }}">
                            <button class="btn btn-warning w-100" type="submit"> Flutter &nbsp; Wave</button></a>
        ...
        @else
            <div class="card">
                <div class="card-body center">
                    <h6>Last updated on <span>{{ date('d M Y', strtotime($order->updated_at)) }} at
                            {{ date('H:i:s', strtotime($order->updated_at)) }}</span> </h6>
                </div>
            </div>
            ...
            <div class="card mt-3">
                ...
                <div class="card-body font-size">
                    ...
                    <div class="row">
                        <div class="col-md-7">
                            Payment – {{ date('d/m/Y', strtotime($order->updated_at)) }}
...
<script>
    $(document).on('submit', '#payment', function(e) {
        ...
        $("#overlay").fadeIn();
        $.ajax({
            url: "{{ route('customCustomerPayment') }}",
            type: 'POST',
            data: formData,
            dataType: "JSON",
            contentType: false,
