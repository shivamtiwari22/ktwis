...
<div class="form-control">
    <form id="store_inventory" enctype="multipart/form-data">
        @csrf
        <div class="row">
            <div class="col-sm-4">
                <label for="featured_image">Image (jpg, jpeg, png, 2MB max)</label>
                <input type="hidden" value="{{$inventory->id}}" name="id" required>
                <input type="hidden" value="{{$inventory->p_id}}" name="p_id" required>
                <input type="file" accept="image/*" name="featured_image"  id="featured_image" class="form-control">
            </div>
        ...
        <div class="row">
            <div class="col-sm-4">
                ...
                <input type="text" name="sku" value="{{$inventory->sku}}" id="sku" required class="form-control">
            </div>
            <div class="col-sm-4">
                <label for="stock_qty">Stock Qty <span class="text-danger">*</span></label>
                <input type="number" value="{{$inventory->stock_qty}}" min="1" name="stock_qty" id="stock_qty" required class="form-control">
            </div>
        </div>
        <div class="row">
            <div class="col-sm-3">
                <label for="purchase_price">Purchase Price <span class="text-danger">*</span></label>
                <input type="number" name="purchase_price" min="1" id="purchase_price" value="{{$inventory->purchase_price}}" required class="form-control">
            </div>
            <div class="col-sm-3">
                <label for="price">Price <span class="text-danger">*</span></label>
                <input type="number" name="price" id="price" min="1" value="{{$inventory->price}}" required class="form-control">
            </div>
            <div class="col-sm-3">
                <label for="offer_price">Offer Price <span class="text-danger">*</span></label>
                <input type="number" name="offer_price" id="offer_price" min="1" value="{{$inventory->offer_price}}" required class="form-control">
            </div>
            <div class="col-sm-3">
                <button type="submit" id="save_inventory" class="btn btn-primary" style="float: right;margin-top: 20px;">Save</button>
            </div>
        </div>
    </form>
</div>
...
<script>
    $(document).ready(function() {
        $('#save_inventory').click(function() {
            ...
            if (validateForm()) {
                var formData = new FormData($('#store_inventory')[0]);
                $.ajax({
                    url: "{{route('vendor.inventory.update_inventory')}}",
                    type: 'POST',
                    data: formData,
                    processData: false,
                    contentType: false,
        ...
        function validateForm() {
            ...
            $('#sku, #stock_qty, #purchase_price, #price, #offer_price ,#featured_image').each(function() {
                var $input = $(this);
                if ($.trim($(this).val()) === ''  &&  $input.attr('id') !== 'featured_image') {
            ...
            $('#featured_image, #sku, #stock_qty, #purchase_price, #price, #offer_price').on('input change', function() {
                $(this).removeClass('is-invalid');
    ...
    $(document).ready(function() {
        $('#featured_image').change(function(e) {
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        $('#imageContainer').html('<img src="' + response.imageDataUrl + '" width="55px" style="border:1px solid black;">');
                    }
                },
                error: function(xhr, status, error) {
                    console.log(xhr.responseText);
